...
<#macro videoList title shouldSearch containerClass>
  ...
  <script id="pagination-table-template" type="text/x-handlebars-template">
    ...
    {{#each pages}}
    <li>
      <a id="page-number-{{this.index}}" class="pagination-page {{this.current}}">{{this.index}}</a>
    </li>
    {{/each}}
  ...
  <script id="video-table-template" type="text/x-handlebars-template">
    <div
      ...
      {{{iceAttrs}}}
    >
      <h3 {{{titleIceAttrs}}}>{{title_t}}</h3>
      <video
        id="vid-table-{{folder-name}}" class="video-table" poster="{{thumbnail_s}}"
        preload="auto"
        {{{videoIceAttrs}}}
      >
      ...
      <div class="grid-info">
        ...
        <div class="lables">
          <p>Tags:
            {{#each tags}}
            <a
              class="generic-tag" onClick="categoryRedirect('{{this.tagName_t}}')"
              title="Go to 'Watch now' for more information"
            >
              {{this.tagName_t}}
            </a>
            {{/each}}
